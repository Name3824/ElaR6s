exports.run = (client, msg) => {
    ...
    emb.setColor('#2040ff');
    emb.setAuthor('About '+client.user.tag, client.user.avatarURL, 'http://vgh.ftp.sh');
    emb.addField('Howdy!', "I'm "+client.user.username+", and as the name says, i'm a damn video game helper!\nI'm built with [node.js](https://nodejs.org), [discord.js](https://discord.js.org) and with "+client.users.get(process.env.OWNER).tag+"'s love aswell!\nIf you want to add me to your guild to suite your gaming needs, click [here](http://vgh.ftp.sh/invite) to invite me!\n*[You need the `MANAGE_SERVER` permission to add me to a guild of your choice]*\nIf you like what i'm doing, please consider [giving my developer a coffee](http://vgh.ftp.sh/donations). VPS costs are high, and we need all the support so that I can be online.");
